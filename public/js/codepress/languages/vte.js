/*
 * CodePress regular expressions for VTE syntax highlighting
 */
...
Language.syntax = [
	...
	{ input : /(&lt;vte:.*?&gt;)/g, output : '<code>$1</code>' }, // vte tag begin
	{ input : /(&lt;\/vte:.*?&gt;)/g, output : '<code>$1</code>' }, // vte tag end
	{ input : /(\{[^\}]*\})/g, output : '<var>$1</var>' } // vte value tag
]

Language.snippets = [
	{ input : 'if', output : '<vte:if test="$0">\n</vte:if>' },
	{ input : 'else', output : '<vte:else>$0\n</vte:else>' },	
	{ input : 'literal', output : '<vte:literal>$0\n</vte:literal>' },
	{ input : 'for', output : '<vte:for key="$0" start="" step="" loop="">\n</vte:for>' },
	{ input : 'foreach', output : '<vte:foreach item="$0" from="" key="" start="" step="" loop="">\n</vte:foreach>' },
	{ input : 'include', output : '<vte:include file="$0" parse="on" />' },
	{ input : 'inchref', output : '<vte:include href="$0" />' },
	{ input : 'params', output : '<vte:params>\n\t<vte:param name="$0" value="" />\n</vte:params>' },
	{ input : 'param', output : '<vte:param name="$0" value="" />' },
	{ input : 'attribute', output : '<vte:attribute name="$0"></vte:attribute>' },
	{ input : 'box', output : '<vte:box module="$0">\n\t<vte:params>\n\t\t<vte:param name="" value="" />\n\t</vte:params>\n\t<vte:template>\n\t\t\n\t</vte:template>\n</vte:box>' },
	{ input : 'template', output : '<vte:template>\n\t\t\n\t</vte:template>' },
	{ input : 'load', output : '<vte:load module="$0" />' },
	{ input : 'value', output : '<vte:value select="$0" />' },
	{ input : 'v', output : '<vte:value select="$0" />' },	
